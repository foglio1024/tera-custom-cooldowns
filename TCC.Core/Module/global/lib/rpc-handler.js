...
class RpcHandler {
    ...
    resetInstance(params) {
        this.mod.networkMod.send("C_RESET_ALL_DUNGEON", 1, {});
        this.debug("Sent C_RESET_ALL_DUNGEON");
    }
    requestPartyInfo(params) {
        this.mod.networkMod.send("C_REQUEST_PARTY_INFO", 2, {
            playerId: params.listingId
        });
        this.debug("Sent C_REQUEST_PARTY_INFO");
    }
    applyToGroup(params) {
        this.mod.networkMod.send("C_APPLY_PARTY", 1, {
            playerId: params.listingId
        });
    ...
    friendUser(params) {
        this.mod.networkMod.send("C_ADD_FRIEND", 1, {
    ...
    unfriendUser(params) {
        if (mod.networkMod.majorPatchVersion >= 103) {
            this.mod.networkMod.send("C_DELETE_FRIEND", 2, {
                playerId: params.playerId
            });
        }
        else {
            this.mod.networkMod.send("C_DELETE_FRIEND", 1, {
                name: params.userName
            });
        }
    ...
    blockUser(params) {
        this.mod.networkMod.send("C_BLOCK_USER", 1, {
            name: params.userName
        });
        this.debug(`Sent C_BLOCK_USER`);
    }
    unblockUser(params) {
        this.mod.networkMod.send("C_REMOVE_BLOCKED_USER", 1, {
            name: params.userName
        });
        this.debug(`Sent C_REMOVE_BLOCKED_USER`);
    }
    setInvitePower(params) {
        this.mod.networkMod.send("C_CHANGE_PARTY_MEMBER_AUTHORITY", 1, {
    ...
    delegateLeader(params) {
        this.mod.networkMod.send("C_CHANGE_PARTY_MANAGER", 2, {
            serverId: params.serverId,
            playerId: params.playerId
        });
    ...
    kickUser(params) {
        this.mod.networkMod.send("C_BAN_PARTY_MEMBER", 1, {
            serverId: params.serverId,
            playerId: params.playerId
        });
    ...
    inspectUser(params) {
        this.mod.networkMod.send("C_REQUEST_USER_PAPERDOLL_INFO", 3, {
            name: params.userName
        });
        this.debug(`Sent C_REQUEST_USER_PAPERDOLL_INFO`);
    }
    inspectUserWithGameId(params) {
        this.mod.networkMod.send("C_REQUEST_USER_PAPERDOLL_INFO_WITH_GAMEID", 3, {
    ...
    groupInviteUser(params) {
        ...
        this.mod.networkMod.send("C_REQUEST_CONTRACT", 1, {
            type: 4,
            name: params.userName,
            data: dataArray
        });
    ...
    guildInviteUser(params) {
        this.mod.networkMod.send("C_INVITE_USER_TO_GUILD", 1, {
            name: params.userName
        });
    ...
    declineBrokerOffer(params) {
        this.mod.networkMod.send("C_TRADE_BROKER_REJECT_SUGGEST", 1, {
            playerId: params.playerId,
    ...
    declineUserGroupApply(params) {
        this.mod.networkMod.send("C_PARTY_APPLICATION_DENIED", 1, {
            pid: params.playerId
        });
        this.debug(`Sent C_PARTY_APPLICATION_DENIED`);
    }
    publicizeListing(params) {
        this.mod.networkMod.send("C_REQUEST_PARTY_MATCH_LINK", 1, {});
        this.debug(`Sent C_REQUEST_PARTY_MATCH_LINK`);
    }
    removeListing(params) {
        this.mod.networkMod.send("C_UNREGISTER_PARTY_INFO", 1, {
            unk1: 20,
            minLevel: 1,
            maxLevel: 65,
            unk3: 3
    ...
    requestListings(params) {
        this.mod.networkMod.send("C_PARTY_MATCH_WINDOW_CLOSED", 1, {});
        ...
        if (min < 1)
            min = 1;
        this.mod.networkMod.send("C_REQUEST_PARTY_MATCH_INFO", 1, {
            minlvl: min,
            maxlvl: max,
            unk2: 3
        });
    ...
    requestListingsPage(params) {
        this.mod.networkMod.send("C_REQUEST_PARTY_MATCH_INFO_PAGE", 1, {
            page: params.page,
            unk1: 3
        });
    ...
    askInteractive(params) {
        this.mod.networkMod.send("C_ASK_INTERACTIVE", 2, {
            unk: 1,
            serverId: params.serverId,
            name: params.userName
        });
    ...
    requestExTooltip(params) {
        this.mod.networkMod.send("C_SHOW_ITEM_TOOLTIP_EX", mod.networkMod.majorPatchVersion >= 103 ? 5 : 4, {
            type: 17,
            dbid: params.itemUid,
            playerId: -1,
    ...
    requestNonDbItemInfo(params) {
        this.mod.networkMod.send("C_REQUEST_NONDB_ITEM_INFO", 2, {
            item: params.itemId
        });
        this.debug(`Sent C_REQUEST_NONDB_ITEM_INFO`);
    }
    registerListing(params) {
        this.mod.networkMod.send("C_REGISTER_PARTY_INFO", 1, {
    ...
    disbandGroup(params) {
        this.mod.networkMod.send("C_DISMISS_PARTY", 1, {});
        this.debug(`Sent C_DISMISS_PARTY`);
    }
    leaveGroup(params) {
        this.mod.networkMod.send("C_LEAVE_PARTY", 1, {});
        this.debug(`Sent C_LEAVE_PARTY`);
    }
    requestListingCandidates(params) {
        this.mod.networkMod.send("C_REQUEST_CANDIDATE_LIST", 1, {});
        this.debug(`Sent C_REQUEST_CANDIDATE_LIST`);
    }
    forceSystemMessage(params) {
        this.mod.networkMod.send("S_SYSTEM_MESSAGE", 1, {
            message: params.message
        });
    ...
    returnToLobby(params) {
        this.mod.networkMod.send("C_RETURN_TO_LOBBY", 1, {});
        this.debug(`Sent C_RETURN_TO_LOBBY`);
    }
    chatLinkAction(params) {
        this.mod.networkMod.send("S_CHAT", 3, {
            channel: 18,
            name: "tccChatLink",
